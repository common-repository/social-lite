...
trait Assets {
    ...
    public static $HANDLE_IFRAME_RESIZER = 'iframe-resizer';
    ...
    public function localizeScript($context) {
        ...
        return [
            ...
            'restQuery' => ['_v' => $this->getPluginConstant(Constants::PLUGIN_CONST_VERSION)],
            'restNonce' => wp_installing() && !is_multisite() ? '' : wp_create_nonce('wp_rest'),
            'publicUrl' => trailingslashit(
                plugins_url('public', $this->getPluginConstant(Constants::PLUGIN_CONST_FILE))
            ),
     ...
     */
    public function enqueueIframeResizer() {
        $useNonMinifiedSources = $this->useNonMinifiedSources();
        $this->enqueueLibraryScript(self::$HANDLE_IFRAME_RESIZER, [
            [$useNonMinifiedSources, 'iframe-resizer/lib/iframe-resizer.child.min.js'],
            'iframe-resizer/lib/iframe-resizer.child.min.js'
        ], [], array(
            'in_footer' => false,
            'strategy'  => 'async',
        ));
    ...
    protected function enqueue(
        ...
        foreach ($src as $s) {
            ...
            $path = path_join($this->getPluginConstant(Constants::PLUGIN_CONST_PATH), $publicSrc);
            if (file_exists($path)) {
                ...
                if ($type === 'script') {
                    ...
                    if (!$isLib) {
                        $this->setLazyScriptTranslations(
                            $useHandle,
                            $this->getPluginConstant(Constants::PLUGIN_CONST_TEXT_DOMAIN),
                            path_join(
                                $this->getPluginConstant(Constants::PLUGIN_CONST_PATH),
    ...
    protected function enqueueComposer(
        ...
        $rootSlug = $this->getPluginConstant(Constants::PLUGIN_CONST_ROOT_SLUG);
        $pluginPath = $this->getPluginConstant(Constants::PLUGIN_CONST_PATH);
        $scriptSuffix = $src === 'index.js' || $src === 'index.css' ? '' : '-' . pathinfo($src, PATHINFO_FILENAME);
...
JS;
    ...
    public function getCachebusterVersion($src, $isLib = false) {
        $default = $this->getPluginConstant(Constants::PLUGIN_CONST_VERSION);
    ...
    public function getPluginsUrl($asset, $isLib = false) {
        return plugins_url(
            ...
            $this->getPluginConstant(Constants::PLUGIN_CONST_FILE)
        );
    }
